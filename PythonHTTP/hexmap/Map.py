...
class Map(object):
    ...
    def __init__(self, size, *args, **keywords):
        #Map size
        rows , cols = size
        self.rows = rows
        self.cols = cols
    ...
    def distance(self, start, destination):
        ...
        logger.debug("diffX: %d, diffY: %d, distance: %d",
                     diffX, diffY, distance)
        return distance
    ...
    def ascii(self, numbers=True):
        ...
        for row in range(self.rows):
            ...
            for col in range(self.cols):
                ...
                if col % 2 == 0:
                    text = "%d,%d" % (row, col) if numbers else ""
    ...
    # def spread(self, origin, radius=1):
    #     """
    #     A slice of a map is a collection of valid cells, starting at an origin, 
    #     and encompassing all cells within a given radius. 
    #     """
    #     result = set((origin, ))  # Start out with this origin cell
    #     neighbors = self.neighbors(origin)  # Get the neighbors for use later
    #     if radius == 1:  # Recursion end case
    #         # Return the set of this cell and its neighbors
    #         result = result | set(neighbors)
    ...
    #     # filter invalid cells before returning.
    ...
    def slice(self, origin, direction, length=2):
        ...
        edge, step = self.directions[direction %
                                     6], self.directions[(direction + 2) % 6]
    ...
    def line(self, origin, direction, length=3):
        ...
        # Work each row, i units out along an edge
        for i in range(1, length + 1):
            results.append((origin[0] + offset[0] * i,
                           origin[1] + offset[1] * i))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-r', '--rows', dest='rows', type=int,
                        default=5, help='Number of rows in grid.  Defaults to 5.')
    parser.add_argument('-c', '--cols', dest='cols', type=int,
                        default=5, help='Number of columns in grid.  Defaults to 5.')
    parser.add_argument('-n', '--numbers', action="store_true", dest="numbers",
                        default=False, help='Display grid numbers on tiles.  Defaults to false.')
    parser.add_argument('-i', '--interactive', action="store_true", dest="interactive",
                        default=False, help="Provide a ncurses interactive interface.")

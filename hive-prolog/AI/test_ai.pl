...
% TESTING MINIMAX
run_minimax_test(R) :-
    two_minimax(step(none, game([piece(4,3,white,[cricket,0])],black,[[pieces_info(white,[queen]),pieces_info(black,[queen])],[],2]),none,continue),-100000000, 100000000,result_selection,next_step_generator,terminal_test,sample_two_player_utility_function,2,R).
